# Analysis and Design of Algorithms

Repository containing the laboratory assignments developed for the Analysis and Design of Algorithms course during the second year of Software Engineering at the University of Málaga (UMA).

### 📚 Course

* Degree: Software Engineering
* University: Universidad de Málaga (UMA)
* Year: 2nd Year
* Course: Analysis and Design of Algorithms
* Academic Year: 2026/2027

### 📂 Labs

The repository contains the different laboratory assignments developed throughout the course.

Lab 1 — Brute Force
* 📁 CountInversionsProblem: Implementation and analysis of algorithms for counting the number of inversions in an array.
* 📁 KthElementProblem: Implementation of algorithms for finding the k-th element of a collection.
* 📁 MatrixMultiplicationProblem: Implementation and analysis of algorithms for multiplying matrices.
  
More labs will be added as the course progresses.

### 🛠️ Technologies

* Java
* Git & GitHub

### 🎯 Focus

The labs focus on the design, implementation, and analysis of algorithms, with particular attention to algorithm efficiency and computational complexity.

### 👩‍💻 Author

María Bonafonte Sánchez

Software Engineering student at the University of Málaga.

⸻

Coursework developed as part of the Analysis and Design of Algorithms course during the second year of Software Engineering.
